...
fn test_render_long_html() {
    create_default_session_globals_then(|| {
        ...
        assert_eq!(
            ((word_cfg("debug_assertions") | word_cfg("windows")) & word_cfg("unix"))
                .render_long_html(),
            "Available on <strong>(debug-assertions enabled or Windows) and Unix\
             </strong> only."
        );
        assert_eq!(
        ...
        assert_eq!(
            name_value_cfg("feature", "std").render_long_html(),
            "Available with <strong>crate feature <code>std</code></strong> only.",
        );
        assert_eq!(
            (name_value_cfg("feature", "sync") & name_value_cfg("feature", "send"))
                .render_long_html(),
            "Available on <strong>crate features <code>sync</code> and <code>send</code></strong> only.",
        );
    })
}
